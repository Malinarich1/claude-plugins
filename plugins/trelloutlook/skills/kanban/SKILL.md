...
Un dev-prompt es una **tarea** con `metadata.tipo: "dev-prompt"`. **Cada uno vive en el tablero de SU proyecto**: al crearlo mandá el `board_id` de ese proyecto, o cae en el principal, donde el otro agente no lo va a buscar. `metadata.prompt_proyecto` **etiqueta y no ubica** — dice de qué habla, no dónde está.
...
- **Ver tu turno:** `python tl.py GET "/tasks?turno_de=<tu lado>&limit=100"`. Una sola llamada, sin `board_id` para que barra **todos** los tableros que ves, y el servidor aplica la regla: los pendientes dirigidos a vos **más** los corridos que pediste vos y esperan que cierres. Cada fila trae `prompt_para`, `prompt_estado`, `prompt_proyecto`, `prompt_prioridad` y `prompt_tarea_id`, así que triás sin abrir ninguno.
  **No armes el turno a mano.** No es "los que me nombran": un prompt contestado vuelve a **quien lo pidió**, y eso no se deduce de `prompt_para` cuando hay más de dos partes. Y ojo con la trampa vieja: **esta lista NO trae `metadata`**, así que filtrar por `.metadata.tipo` sobre ella devuelve **siempre vacío**. Para la metadata entera, `GET /tasks/{id}`.
- **Dejar uno:** `POST /tasks` con el **`board_id` del proyecto del que habla el pedido** y `metadata` = `{"tipo":"dev-prompt","prompt_para":"<el otro lado>","prompt_estado":"pendiente","prompt_proyecto":"…"}`, más una descripción con la forma exacta de lo que necesitás (endpoint, campos, ejemplo) y qué ya está hecho de tu lado. `prompt_proyecto` **etiqueta, no ubica**: si omitís `board_id`, el prompt y su tarjeta caen en el principal aunque la etiqueta diga otra cosa. Sin `column_id` nace en el ABIERTO de ese tablero.
